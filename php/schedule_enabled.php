...
if ( flock($fp, LOCK_EX) ) {
	...
	// $schedule_object から受け取った位置の enabled キーの値を変更する

	$post_index = (int) $_POST["index"];
	$post_enabled = filter_var($_POST["enabled"] ?? "", FILTER_VALIDATE_BOOLEAN);
	$schedule_object[$post_index]["enabled"] = $post_enabled;
